...
use qrap_da::{Blob, MockDaClient, Namespace};
use qrap_fee_splitter::{FeeSplitter, ProverInfo, ValidatorInfo};
use qrap_fpga::{FpgaProver, MockFpga, PlonkInput, PowerState};
use qrap_net::NodeId;
...
impl Simulation {
    ...
    pub async fn run(&mut self) -> SimMetrics {
        ...
        info!(
            "║  Byzantine: {:.1}% {:<37}║",
            self.config.byzantine_ratio * 100.0,
            ""
        );
        info!(
            "║  Partition: {:.1}% {:<37}║",
            self.config.partition_ratio * 100.0,
            ""
        );
        info!(
            "║  DA Fail:   {:.1}% {:<37}║",
            self.config.da_failure_rate * 100.0,
            ""
        );
        info!(
            "║  FPGA Degradation: {:<32}║",
            if self.config.fpga_degradation {
                "ON"
            } else {
                "OFF"
            }
    ...
    async fn generate_transactions(&mut self) {
        ...
        info!(
            "  OK Processed: {}, Failed: {}, Byzantine detected: {}",
    ...
    async fn run_consensus(&mut self) {
        ...
        info!(
            "  OK Rounds: {}, Blocks: {}",
            self.metrics.consensus_rounds, self.metrics.blocks_proposed
        );
    }
    ...
    async fn generate_proofs(&mut self) {
        ...
        info!(
            "  OK Proofs: {}, Energy: {}mJ",
            self.metrics.fpga_proofs, self.metrics.fpga_energy_mj
        );
    }
    ...
    async fn submit_to_da(&mut self) {
        ...
        for i in 0..10 {
            ...
            let blob = Blob::new(Namespace::new(b"qrap-sim"), data);
        ...
        info!(
            "  OK Submitted: {}, Failed: {}",
            self.metrics.da_blobs_submitted, self.metrics.da_blobs_failed
        );
    }
    ...
    async fn distribute_fees(&mut self) {
        ...
        let provers: Vec<ProverInfo> = self
            .agents
            .iter()
            .map(|a| ProverInfo {
        ...
        let split = self
            .fee_splitter
            .split(total_fees, &provers, &validators)
            .unwrap();

        info!("  OK Fees distributed:");
        info!(
            "    Provers:    {} (bonus: {})",
            split.provers, split.prover_bonus
        );
    ...
    fn print_results(&self) {
        ...
        info!(
            "║  Blocks:           {:<32} ║",
            self.metrics.blocks_proposed
        );
        info!("║  Txs Processed:    {:<32} ║", self.metrics.txs_processed);
        info!("║  Txs Failed:       {:<32} ║", self.metrics.txs_failed);
        info!(
            "║  Byzantine Detect: {:<32} ║",
            self.metrics.byzantine_detected
        );
        info!("║  FPGA Proofs:      {:<32} ║", self.metrics.fpga_proofs);
        info!(
            "║  FPGA Energy:      {:<32}mJ ║",
            self.metrics.fpga_energy_mj
        );
        info!(
            "║  DA Submitted:     {:<32} ║",
            self.metrics.da_blobs_submitted
        );
        info!(
            "║  DA Failed:        {:<32} ║",
            self.metrics.da_blobs_failed
        );
        info!(
            "║  Partitions Healed:{:<32} ║",
            self.metrics.partition_healed
        );
        info!("╚══════════════════════════════════════════════════════╝");
    }
}
