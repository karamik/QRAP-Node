...
impl Default for PipelineStats {
    fn default() -> Self {
        Self { ntt_runs: 0, msm_runs: 0, field_runs: 0, total_proofs: 0, avg_ntt_ms: 0.0, avg_msm_ms: 0.0, avg_field_ms: 0.0 }
    }
}
...
impl AwsF1Prover {
    ...
    fn fpga_prove(&mut self, input: &PlonkInput) -> Result<PlonkProof, FpgaError> {
        let start = Instant::now();
        let dev = self.device.as_ref().ok_or_else(|| FpgaError::NotAvailable("Device not initialized".to_string()))?;
        ...
        ntt_buf.as_mut_slice()[..ntt_data.len().min(4096)].copy_from_slice(&ntt_data[..ntt_data.len().min(4096)]);
        ntt_buf.sync_to_device().map_err(|e| FpgaError::ProofFailed(format!("NTT sync: {}", e)))?;
        ...
        ntt_run.set_arg(0, &ntt_buf).map_err(|e| FpgaError::ProofFailed(format!("NTT arg: {}", e)))?;
        ntt_run.start().map_err(|e| FpgaError::ProofFailed(format!("NTT start: {}", e)))?;
        ntt_run.wait(10000).map_err(|e| FpgaError::ProofFailed(format!("NTT timeout: {}", e)))?;
        
        let ntt_time = ntt_start.elapsed();
        self.pipeline_stats.ntt_runs += 1;
        self.pipeline_stats.avg_ntt_ms = (self.pipeline_stats.avg_ntt_ms * (self.pipeline_stats.ntt_runs - 1) as f64 + ntt_time.as_millis() as f64) / self.pipeline_stats.ntt_runs as f64;
        ...
        let msm_data = bincode::serialize(&input.commitments)
            .map_err(|e| FpgaError::ProofFailed(e.to_string()))?;
        msm_buf.as_mut_slice()[..msm_data.len().min(4096)].copy_from_slice(&msm_data[..msm_data.len().min(4096)]);
        msm_buf.sync_to_device().map_err(|e| FpgaError::ProofFailed(format!("MSM sync: {}", e)))?;
        ...
        msm_run.set_arg(0, &msm_buf).map_err(|e| FpgaError::ProofFailed(format!("MSM arg: {}", e)))?;
        msm_run.start().map_err(|e| FpgaError::ProofFailed(format!("MSM start: {}", e)))?;
        msm_run.wait(10000).map_err(|e| FpgaError::ProofFailed(format!("MSM timeout: {}", e)))?;
        
        let msm_time = msm_start.elapsed();
        self.pipeline_stats.msm_runs += 1;
        self.pipeline_stats.avg_msm_ms = (self.pipeline_stats.avg_msm_ms * (self.pipeline_stats.msm_runs - 1) as f64 + msm_time.as_millis() as f64) / self.pipeline_stats.msm_runs as f64;
        ...
        let field_data = bincode::serialize(&input.public_inputs)
            .map_err(|e| FpgaError::ProofFailed(e.to_string()))?;
        field_buf.as_mut_slice()[..field_data.len().min(4096)].copy_from_slice(&field_data[..field_data.len().min(4096)]);
        field_buf.sync_to_device().map_err(|e| FpgaError::ProofFailed(format!("Field sync: {}", e)))?;
        ...
        field_run.set_arg(0, &field_buf).map_err(|e| FpgaError::ProofFailed(format!("Field arg: {}", e)))?;
        field_run.start().map_err(|e| FpgaError::ProofFailed(format!("Field start: {}", e)))?;
        field_run.wait(10000).map_err(|e| FpgaError::ProofFailed(format!("Field timeout: {}", e)))?;
        
        let field_time = field_start.elapsed();
        self.pipeline_stats.field_runs += 1;
        self.pipeline_stats.avg_field_ms = (self.pipeline_stats.avg_field_ms * (self.pipeline_stats.field_runs - 1) as f64 + field_time.as_millis() as f64) / self.pipeline_stats.field_runs as f64;
        debug!("Stage 3 Field: {}ms", field_time.as_millis());
        
        field_buf.sync_from_device().map_err(|e| FpgaError::ProofFailed(format!("Result sync: {}", e)))?;
...
impl FpgaProver for AwsF1Prover {
        ...
        match XrtDeviceHandle::open(self.device_index) {
            Some(dev) => {
                ...
                if let Err(rc) = dev.load_xclbin(&self.xclbin_path) {
                    warn!("Failed to load xclbin (rc={}), falling back to software", rc);
                    ...
                    if let Some(dev_ref) = self.device.as_ref() {
                        ...
                        let loaded = [self.ntt_kernel.is_some(), self.msm_kernel.is_some(), self.field_kernel.is_some()];
                        if loaded.iter().all(|&x| x) {
                            info!("All 3 kernels loaded: NTT, MSM, Field");
                        } else {
                            warn!("Some kernels missing: NTT={}, MSM={}, Field={}", 
    ...
    async fn prove(&mut self, input: PlonkInput) -> Result<PlonkProof, FpgaError> {
        let all_kernels = self.ntt_kernel.is_some() && self.msm_kernel.is_some() && self.field_kernel.is_some();
        
        if all_kernels {
            debug!("AWS F1 3-stage pipeline: {} nullifiers, {} commitments", 
                   input.nullifiers.len(), input.commitments.len());
            let result = self.fpga_prove(&input)?;
            self.health.scrubber_cycles += 1;
            info!("AWS F1 proof: {}ms, {}mJ | Pipeline: NTT={:.0}ms MSM={:.0}ms Field={:.0}ms",
                  result.generation_time_ms, result.power_consumed_mj,
    ...
    async fn set_power(&mut self, state: PowerState) -> Result<(), FpgaError> {
        ...
        info!("AWS F1 power state -> {:?} ({}W)", state, state.power_watts());
        Ok(())
    }
