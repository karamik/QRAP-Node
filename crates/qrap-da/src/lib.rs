...
impl DaClient for MockDaClient {
    async fn submit_blob(&self, blob: Blob) -> Result<Hash, DaError> {
        ...
        *height += 1;
        ...
        info!("MockDA: blob submitted, commitment={}, height={}", 
              hex::encode(&commitment[..8]), *height);
        Ok(commitment)
    }
    ...
    async fn get_blob(&self, commitment: &Hash, namespace: &Namespace) -> Result<Blob, DaError> {
        ...
        blobs.get(commitment)
            .cloned()
            .filter(|b| &b.namespace == namespace)
            .ok_or_else(|| DaError::BlobNotFound(hex::encode(commitment)))
    }
    
    async fn sample(&self, _block_height: u64, confidence_threshold: f64) -> Result<DasResult, DaError> {
        ...
        info!("MockDA: DAS complete, confidence={:.4}%, {}ms", confidence * 100.0, duration);
        Ok(result)
    }
    ...
    async fn verify_inclusion(&self, proof: &InclusionProof) -> Result<bool, DaError> {
        ...
        debug!("MockDA: inclusion verification, commitment={}, exists={}", 
               hex::encode(&proof.blob_commitment[..8]), exists);
    ...
    pub fn verify_header_chain(&self, headers: &[BlobstreamHeader]) -> Result<bool, DaError> {
        ...
        for header in headers {
            ...
            if computed != header.data_root {
                warn!("Blobstream: header hash mismatch at height {}", header.height);
                return Ok(false);
            }
