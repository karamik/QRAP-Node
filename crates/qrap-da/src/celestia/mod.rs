...
use crate::{DaClient, Blob, Namespace, InclusionProof, DasResult, DaError};
...
impl CelestiaClient {
    ...
    pub async fn submit_pfb(&self, blob: &Blob) -> Result<String, DaError> {
        info!("Celestia: submitting blob, namespace={}, size={} bytes", 
              blob.namespace.as_hex(), blob.size_bytes());
    ...
    pub async fn get_blob(&self, commitment: &[u8], height: u64) -> Result<Vec<u8>, DaError> {
        info!("Celestia: retrieving blob at height={}, commitment={}", 
              height, hex::encode(&commitment[..8]));
...
impl DaClient for CelestiaClient {
    async fn submit_blob(&self, blob: Blob) -> Result<crate::Hash, DaError> {
        ...
        info!("Celestia: blob confirmed, tx={}, commitment={}", 
              tx_hash, hex::encode(&commitment[..8]));
        Ok(commitment)
    }
    
    async fn get_blob(&self, commitment: &crate::Hash, namespace: &Namespace) -> Result<Blob, DaError> {
        let height = self.get_latest_height().await?;
        let data = self.get_blob(commitment, height).await?;
        Ok(Blob::new(namespace.clone(), data))
    }
    
    async fn sample(&self, block_height: u64, confidence_threshold: f64) -> Result<DasResult, DaError> {
        let result = self.perform_das(block_height).await?;
        if result.confidence < confidence_threshold {
            return Err(DaError::DasInsufficient(result.confidence, confidence_threshold));
        }
        Ok(result)
    }
    ...
    pub fn verify_header_chain(&self, headers: &[BlobstreamXHeader]) -> Result<bool, DaError> {
        ...
        for header in headers {
            if header.height != self.trusted_height + 1 {
                warn!("Blobstream: non-sequential header, expected={}, got={}", 
                      self.trusted_height + 1, header.height);
                return Ok(false);
            }
        ...
        info!("Blobstream: verified {} headers from height={}", headers.len(), self.trusted_height);
        Ok(true)
    }
